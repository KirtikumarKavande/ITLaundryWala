...
const ClothType = () => {
  ...
  <tr>
    <th scope="col" className="px-2  md:px-auto text-center text-white py-3">
      Sr
    </th>
    <th scope="col" className=" px-14 md:mr-5  text-white md:px-auto py-3">
      Cloth Type
    </th>
    <th scope="col" className="text-white  md:text-left   px-6 md:px-auto py-3">
      <span className="md:pl-8">Qty</span>
    </th>
    <th scope="col" className=" px-4 text-white text-center md:px-auto py-3 "> 
      Amount
    </th>
  </tr>
</thead>
  ...
  {numberOFCloth.map((item, index) => {
    return (
      <tr className="relative text-black bg-blue-500 border-b">
        ...
        <td className="px-1  py-4 z-50 absolute" style={{ zIndex: Math.abs(zIndex--) }}>
          <SelectClothType key={item} />
        </td>
        <td className="px-3 md:px-auto  py-4">
          <input className="bg-gray-200 appearance-none border-2  border-gray-200 rounded w-14 md:w-36 h-8 py-2 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-purple-500" />
        </td>
