...
const WashTypeButtons = ({setCurrentPage}) => {
  const selectedWashType=(data)=>{
   ...
   {
    setCurrentPage("orderSummeryAndCustomerDetails")
   }
  }
  return (
    <div>
      <div className="space-y-6 flex-col   w-full mx-auto bg-[#F8F7F5] pb-16 pt-6 ">
        {washTypeButtonMenu.map((item) => (
          <button onClick={()=>{selectedWashType(item)}} className=" flex w-[27vw] h-12 items-center justify-between px-4 mx-auto border border-black  rounded-full text-black  space-x-7 tracking-wide">
            {" "}
            <div>
