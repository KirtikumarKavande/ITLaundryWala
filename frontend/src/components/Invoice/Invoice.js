        ...
        <div className="actual-receipt mx-auto w-[700px]" >
          ...
          <div className="container  ">
            ...
            <main className="mt-28">
              <div className="flex justify-between mt-5">
                <div >
                  <div className="flex pb-5">
                    ...
                    <p>&nbsp;<span>{splittePickupdDate[0]}</span> <span>{MonthNames[+splittePickupdDate[1]-1]},</span> <span>{splittePickupdDate[2]}</span> </p>
                  </div>
                  <h3 className="text-lg font-bold">Billed to:</h3>
                  <p className="mt-2">Cus ID: <span>{userDetails?.customerId}</span></p>
                  <p className="mt-2 ">Name  : <span>{userDetails?.name}</span></p>
                  <p className="mt-2">Mo No: <span>{userDetails?.mobileNumber}</span></p>
                </div>
                <div cla>
                  <div className="flex pb-5">
                    <p className="font-bold">D.O.D:</p>
                    <p>&nbsp;<span>{splitteDeliverydDate[0]}</span> <span>{MonthNames[+splitteDeliverydDate[1]-1]},</span> <span>{splitteDeliverydDate[2]}</span></p>
                  </div>
                  <h3 className="text-lg font-bold">From:</h3>
                  <p className="mt-2">I.T LaundryWala</p>
                  <p className="mt-2"><p>Mukai Nagar, Near Matoshree</p>  <p>Food Court, Hinjewadi</p>  </p>
                  ...
                  <tbody>
                    {
                      invoiceData?.clothType.map((item)=>(
                        ...
                        <td className="px-2 py-1">{invoiceData?.washType?.key}</td>
              ...
              <div className="flex  mt-4 bg-[#E6E6E8] pr-3 justify-end space-x-16 pb-1 ">
                <div className=" flex items-center ">
                  <h3 className="text-lg font-bold pr-1">Weight:</h3>
                  <p>{invoiceData?.weight||'-'}{invoiceData?.weight?" kg":""} </p>
                </div>
                <div className=" flex items-center pr-10 ">
                  <h3 className="text-lg font-bold pr-1">Total:&nbsp;</h3>
                  <p>{invoiceData?.amountForPerPeice>0?invoiceData?.amountForPerPeice:invoiceData?.amountForPerKg}</p>
                </div>
              </div>
